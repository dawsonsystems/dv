...
import uk.co.devooght.auth.LocalLogon
...
class ShiroDbRealm {
 ...
 def shiroPermissionResolver
    ...
    def authenticate(authToken) {
        ...
        // exception.
        logon = LocalLogon.findByUsername(username)
        if (!logon) {
            throw new UnknownAccountException("No account found for user [${username}]")
        }

        log.info "Found user '${logon.username}' in DB"

        // Now check the user's password against the hashed value stored
        // in the database.
        account = new SimpleAccount(username, logon.passwordHash, ShiroDbRealm.name)
...
//        def roles = Profile.withCriteria {
//            roles {
//                eq("name", roleName)
//            }
//
